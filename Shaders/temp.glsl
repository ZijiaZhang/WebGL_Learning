...
float shadow(vec3 location, vec3 toLight) {
  vec2 tCube0 = intersectCube(location, toLight, Cube0Min, Cube0Max);
  if (tCube0.x > 0.0 && tCube0.x < 0.9999 && tCube0.x < tCube0.y) return 0.0;
  return 1.0;
}
vec3 calculatedColor(vec3 origin, vec3 ray, vec3 light) {
  vec3 colorMask = vec3(1.0);
  vec3 accumulatedColor = vec3(0.0);
  for (int bounce = 0; bounce < 100; bounce++) {
    vec2 tRoom = intersectCube(origin, ray, roomCubeMin, roomCubeMax);
    vec2 tCube0 = intersectCube(origin, ray, Cube0Min, Cube0Max);
    float t = 1000.0;
    ...
    if (t == tRoom.y) {
      ...
      if (hit.x < -0.9999) surfaceColor = vec3(1.0, 0.0, 0.0);
      else if (hit.x > 0.9999) surfaceColor = vec3(0.0, 1.0, 0.0);
      ray = uniformlyRandomDirection(timeSinceStart);
    } else if (t == 1000.0) {
      break;
    } else {
      if (false) {} else if (t == tCube0.x && tCube0.x < tCube0.y) normal = normalForCube(hit, Cube0Min, Cube0Max);
